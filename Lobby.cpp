...
Lobby::Lobby(int id, int playerAmount) : amount(playerAmount), state(WAITING), currentToken(0), lastStep(Step()),
                                         lobby_id(id) { }
...
int Lobby::serverState() {
//
    return state;
}


bool Lobby::isLocked(int token) {
    assert (0 <= token && token <= 1000);
    return token != currentToken;
}

int Lobby::registerPlayer(int token) {
    assert (0 <= token && token <= 1000);
...
bool Lobby::makeStep(int token, Step step) {
